...
def alphabeta_min_node(board, color, alpha, beta, limit, caching = 0, ordering = 0):
    ...
    if not moves:
        if caching:
            cache_alphabeta[(board, 3 - color)] = min_move, compute_utility(board, color)
            return cache_alphabeta[(board, 3 - color)]
        return min_move, compute_utility(board, color)
...
def alphabeta_max_node(board, color, alpha, beta, limit, caching = 0, ordering = 0):
    ...
    if not moves:
        if caching:
            cache_alphabeta[(board, color)] = max_move, compute_utility(board, color)
            return cache_alphabeta[(board, color)]
        return max_move, compute_utility(board, color)
...
def node_ordering_sort(board, color):
    ...
    for i in range(1, len(arr)):
        ...
        print (arr[j])
        print (key)
        while (j >= 0 and (key > int(compute_utility(play_move(board, color, arr[j][0], arr[j][1]), color)))):
            arr[j + 1] = arr[j]
            j -= 1
        arr[j + 1] = key
    return arr
